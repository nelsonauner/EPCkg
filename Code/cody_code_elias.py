...
os.chdir("/Users/elias/Documents/programming/TGG predictive/Kick/Raw Data")
training = pd.read_csv('training.csv')
#test = pd.read_csv('Raw_data/test.csv')
reliability = pd.read_csv('reliability_external.csv')


 # Exploratory 
training['IsBadBuy'].value_counts() #64,007 No, 8976 Yes. 14%
training.VehBCost[training.IsBadBuy==1].describe()
training.IsBadBuy[training.VehBCost > 9000].value_counts() #above 12,000 it is half kicks. Almost all kicks above 15,000

#Create histograms
def histogramit(data, howfar):
    plt.clf
    plt.hist([data,data[training.IsBadBuy==1]], 20,range =howfar, normed=1, label=['ok','kicks'],color=['green','red'], alpha=0.5)
    legend(prop={'size': 10})
    plt.show()

histogramit(training.discount,[-5000,5000])
histogramit(training.VehicleAge,[0,10])
histogramit(training.VehBCost,[0,15000])

#Create a scatter or IsBadBuy against VehicleAge
ptvage= training.pivot_table('IsBadBuy', rows='VehicleAge', aggfunc=mean)
ptvage = ptvage.reset_index()
plt.scatter(ptvage.VehicleAge, ptvage.IsBadBuy)

#Create a bar chart of share bad by brand
ptmake= training.pivot_table('IsBadBuy', rows='Make', aggfunc=mean)
ptmake = ptmake.reset_index()

fig = plt.figure()
ax = plt.subplot(111)
width=.8
ax.bar(range(len(ptmake.Make)), ptmake.IsBadBuy, width=width)
ax.set_xticks(np.arange(len(ptmake.Make)) + width/2)
ax.set_xticklabels(ptmake.Make, rotation=90)
plt.show()

#A few additional statistics
plt.scatter(training.VehBCost,training.IsBadBuy)

training.pivot_table('IsBadBuy', rows='AUCGUART', aggfunc=mean)
numpy.corrcoef(training['IsBadBuy'],training['VehBCost'])
...
regression_OLS=sm.OLS(data_train.y,DataFrame(data_train[covariates]),hasconst=True).fit()
